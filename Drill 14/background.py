...
class TileBackground:
    ...
    def __init__(self):
        ...
        self.windows_left = 0
        self.windows_bottom = 0

    def update(self):
        pass

    def draw(self):
        self.windows_left = clamp(0, int(server.boy.x) - self.canvas_width // 2,
                                  self.w - self.canvas_width - 1)
        self.windows_bottom = clamp(0, int(server.boy.y) - self.canvas_height // 2,
                                    self.h - self.canvas_height - 1)

        tile_left = self.windows_left // 800
        tile_right = (self.windows_left * self.canvas_width) // 800
        left_offset = self.windows_left % 800

        tile_bottom = self.windows_bottom // 600
        tile_top = (self.windows_bottom * self.canvas_height) // 600
        bottom_offset = self.windows_bottom % 600
